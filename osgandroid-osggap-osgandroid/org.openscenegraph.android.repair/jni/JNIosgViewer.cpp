...
JNIEXPORT void JNICALL Java_org_openscenegraph_osg_viewer_Viewer_nativeAddEventHandler(
                JNIEnv *env, jclass, jlong cptr, jlong cptrpicker) {
	osgViewer::Viewer *v = reinterpret_cast<osgViewer::Viewer*> (cptr);
	osgGA::GUIEventHandler *p =
			reinterpret_cast<osgGA::GUIEventHandler*> (cptrpicker);
	v->addEventHandler(p);
}

JNIEXPORT jdouble JNICALL Java_org_openscenegraph_osg_viewer_Viewer_nativeGetFPS(
                JNIEnv *env, jclass, jlong cptr) {
	osgViewer::Viewer *v = reinterpret_cast<osgViewer::Viewer*> (cptr);

	double fps = 0.0;
	if( v->getViewerStats()->getAveragedAttribute("Frame rate", fps, true) )
	{
		return fps;
	}
	else
...
/*JNIEXPORT void JNICALL Java_org_openscenegraph_osg_viewer_Viewer_nativeTouchMoveEvent(JNIEnv *env, jclass, jlong cptr, jfloat x, jfloat y)
{
    osgViewer::Viewer *v = reinterpret_cast<osgViewer::Viewer*> (cptr);
    if (v == NULL)
        return;

    LOGI("TouchMoveEvent : (%f %f)\n",x,y);

    v->getEventQueue()->mouseMotion(x, y);
}

JNIEXPORT void JNICALL Java_org_openscenegraph_osg_viewer_Viewer_nativeTouchPressEvent(JNIEnv *env, jclass, jlong cptr, jfloat x, jfloat y, jint button)
{
    osgViewer::Viewer *v = reinterpret_cast<osgViewer::Viewer*> (cptr);
    if (v == NULL)
        return;

    LOGI("TouchPressEvent : (%f %f)\n",x,y);

    v->getEventQueue()->mouseButtonPress(x, y, button);
}

JNIEXPORT void JNICALL Java_org_openscenegraph_osg_viewer_Viewer_nativeTouchReleaseEvent(JNIEnv *env, jclass, jlong cptr, jfloat x, jfloat y, jint button)
{
    osgViewer::Viewer *v = reinterpret_cast<osgViewer::Viewer*> (cptr);
    if (v == NULL)
        return;

    LOGI("TouchReleaseEvent : (%f %f)\n",x,y);
...
//add
//JNIEXPORT void JNICALL Java_org_openscenegraph_osg_viewer_DisplaySettings_nativeSetViewMatrix(JNIEnv *env, jclass, jdoubleArray viewMatrix)
//{
//	jdouble *matrix;
//
//	matrix = env->GetDoubleArrayElements(viewMatrix, JNI_FALSE);
//
//    osg::Matrix viewMatrixd(matrix);
//
//        __android_log_print(ANDROID_LOG_INFO,"nativeSetViewMatrix",
//        		"%.2lf %.2lf %.2lf %.2lf \n%.2lf %.2lf %.2lf %.2lf \n%.2lf %.2lf %.2lf %.2lf \n%.2lf %.2lf %.2lf %.2lf",
//        		viewMatrixd(0,0), viewMatrixd(0,1), viewMatrixd(0,2), viewMatrixd(0,3),
//        		viewMatrixd(1,0), viewMatrixd(1,1), viewMatrixd(1,2), viewMatrixd(1,3),
//        		 viewMatrixd(2,0), viewMatrixd(2,1), viewMatrixd(2,2), viewMatrixd(2,3),
//        		 viewMatrixd(3,0), viewMatrixd(3,1), viewMatrixd(3,2), viewMatrixd(3,3));
//
//    env->ReleaseDoubleArrayElements(viewMatrix, matrix, JNI_ABORT);
//}
}
