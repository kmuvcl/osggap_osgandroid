...
public class Node implements Native {
	...
	private static native long nativeGetBound(long cptr);
	private static native long nativeGetParent(long cptr, int i);
	
	private static native void nativeLightingOff(long cptr);
	...
	public BoundingSphere getBound() {
		long cptr = nativeGetBound(_cptr);
		if(cptr == 0)
			return null;
		return new BoundingSphere(cptr);
	}
	
	public Group getParent(int i) {
		long cptr = nativeGetParent(_cptr, i);
		if(cptr == 0)
			return null;
		return new Group(cptr);
	}
	
	public void lightingOff() {
		nativeLightingOff(_cptr);
	}
